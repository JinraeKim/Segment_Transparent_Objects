...
def parse_args_ros():
    """
    With default options for ROS application
    """
    parser = argparse.ArgumentParser(description='Segmentron')
    parser.add_argument('--config-file', metavar="FILE",
                        default="configs/trans10K/translab.yaml",
                        help='config file path')
    # cuda setting
    parser.add_argument('--no-cuda', action='store_true', default=False,
                        help='disables CUDA training')
    ...
    args = parser.parse_args()
    args.opts = ['TEST.TEST_MODEL_PATH', './demo/16.pth', 'DEMO_DIR', './demo/imgs']  # default setting
